...
class WorkflowContractTests(unittest.TestCase):
    def test_generated_numbered_workflows_are_dispatch_only_and_drift_free_without_checkout_mutation(self) -> None:
        ...
        for content in generated.values():
            ...
            self.assertEqual(jobs["aggregate"]["needs"], wave_ids)
            self.assertNotIn("schedule:", text)
            self.assertNotIn("cron:", text)
            self.assertIn("type: string", text)
            self.assertIn("required: true", text)
            self.assertNotIn("default: I_HAVE_WRITTEN_AUTHORIZATION", text)
            self.assertIn("if: ${{ inputs.authorization_acknowledgement == 'I_HAVE_WRITTEN_AUTHORIZATION'", text)
            ...
            self.assertIn("class_b_block_start:", text)
            self.assertIn("- '64'", text)
            self.assertNotIn("fromJSON(inputs.class_b_block_start)", text)
            ...
            for wave, wave_id in enumerate(wave_ids):
                ...
                if wave == 0:
                    ...
                    self.assertEqual(job["needs"], wave_ids[wave - 1])
                self.assertEqual(job["if"], "${{ inputs.authorization_acknowledgement == 'I_HAVE_WRITTEN_AUTHORIZATION' && github.ref == format('refs/heads/{0}', github.event.repository.default_branch) }}")
                self.assertEqual(job["environment"], "internet-scan")
                self.assertEqual(job["permissions"], {"contents": "read"})
                self.assertEqual(job["timeout-minutes"], 360)
                self.assertEqual(job["strategy"]["max-parallel"], "${{ fromJSON(inputs.matrix_max_parallel) }}")
                self.assertEqual(job["strategy"]["matrix"]["classb_offset"], [0, 1, 2, 3])
                self.assertEqual(job["strategy"]["matrix"]["classa"], ["${{ inputs.class_a }}"])
                ...
                self.assertEqual(scan_step["env"]["BLOCK_START"], "${{ inputs.class_b_block_start }}")
            ...
            self.assertIn("--class-b-block-start ${{ inputs.class_b_block_start }}", text)
